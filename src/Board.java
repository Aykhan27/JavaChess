...
public class Board {
	...
	static void startGame() {
		...
		System.out.println("\tFor bishops, knights, and rooks, use \"Q\" or \"K\" to specify Queen's or King's side.");
		System.out.println("\tProvide a space and then enter a valid tile. For example, \"bishopK c4\"");
		System.out.println("\tPawns automatically promote to queens. The new queens are referenced by the pawn's file. For example, \"queenH\"");
		System.out.println("\tTo castle, type \"castle\", followed by a space and then a capital \"K\" or \"Q\" to specify a side. For example, \"castle Q\"\n");
	...
	public static boolean isPathClear(int x1, int y1, int x2, int y2) {
		...
		if (xDistance < 0) xDir = -1;
		else if (xDistance > 0) xDir = 1;

		if (yDistance < 0) yDir = -1;
		else if (yDistance > 0) yDir = 1;

		if (xDistance != 0)  size = Math.abs(xDistance) - 1;
		else size = Math.abs(yDistance) - 1;
		
		// change on x and y
		for (int i = 0; i < size; i++) {
			x1 += xDir;
			y1 += yDir;
			if (getPiece(x1, y1) != null) return false;
		}
		return true;
	}
